...
class TestMaterializeGraph():
    def test_materialize_graph_from_file_with_ds(self):
        ...
        with patch("builtins.open", mock_open(read_data="data")) as mock_file:
            with requests_mock.Mocker() as m:
                m.post("http://localhost:5820/admin/virtual_graphs/import_db", status_code=204)
    ...
    def test_materialize_graph_from_file_with_bad_ds(self):
        ...
        with patch("builtins.open", mock_open(read_data="data")) as mock_file:
            with requests_mock.Mocker() as m:
                m.post("http://localhost:5820/admin/virtual_graphs/import_db", status_code=404, text="Data Source 'ds_sd_int_test' Not Found!")
    ...
    def test_materialize_graph_payload(self):
        ...
        def text_callback(request,context):
            assert request.text == '{"db": "db_test", "mappings": "data", "named_graph": null, "remove_all": false, "options": {"mappings.syntax": "SMS2"}, "data_source": "ds_test"}'
        ...
        with patch("builtins.open", mock_open(read_data="data")) as mock_file:
            with requests_mock.Mocker() as m:
                m.post("http://localhost:5820/admin/virtual_graphs/import_db", status_code=200, text=text_callback)
...
class TestContentType():
    ...
    def test_guess_import_format(self):
        ...
        assert guess_import_format('test.tsv') == (None, 'text/tab-separated-values', 'DELIMITED', '\t')
        assert guess_import_format('test.json') == (None, 'application/json', 'JSON', None)
        assert guess_import_format('test.what') == (None, None, None, None)

        assert guess_import_format('test.csv.gz') == ('gzip', 'text/csv', 'DELIMITED', ',')
        assert guess_import_format('test.csv.zip') == ('zip', 'text/csv', 'DELIMITED', ',')
        assert guess_import_format('test.csv.bz2') == ('bzip2', 'text/csv', 'DELIMITED', ',')
...
class TestContent():
    ...
    def test_import_file(self):
        ...
        m = content.ImportFile('test.delimited', content_type="text/delimited", input_type='DELIMITED',
                               name='Override.myformat', separator=':')
