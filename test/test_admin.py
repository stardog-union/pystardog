...
@pytest.mark.skip(reason="mappings return random bytes so we can't really detect"
                         "if the custom mappings are passed or not, https://github.com/stardog-union/pystardog/issues/69")
def test_create_vg_with_custom_mappings(admin, music_options):
    ...
    # assert mappings should exists and should be different from the autogenerated ones
    vg.delete()
